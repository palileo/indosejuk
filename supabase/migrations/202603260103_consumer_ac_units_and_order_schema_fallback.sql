...
alter table if exists public.orders
    ...
    add column if not exists ac_unit_key text;
...
declare
    ...
    profile_referral text := nullif(trim(coalesce(meta->>'referral', meta->>'referal', '')), '');
    profile_ac_units jsonb := case
        when jsonb_typeof(coalesce(meta->'ac_units', meta->'acUnits')) = 'array' then coalesce(meta->'ac_units', meta->'acUnits')
        else '[]'::jsonb
    end;
...
begin
    ...
    set role = excluded.role,
        ...
        referral = coalesce(nullif(trim(excluded.referral), ''), public.profiles.referral),
        ac_units = case
            when coalesce(jsonb_array_length(public.profiles.ac_units), 0) > 0 then public.profiles.ac_units
            when coalesce(jsonb_array_length(excluded.ac_units), 0) > 0 then excluded.ac_units
            else public.profiles.ac_units
        end,
...
set referral = coalesce(
        p.referral,
        nullif(trim(coalesce(u.raw_user_meta_data->>'referral', u.raw_user_meta_data->>'referal', '')), '')
    ),
    ac_units = case
        when coalesce(jsonb_array_length(p.ac_units), 0) > 0 then p.ac_units
        when jsonb_typeof(coalesce(u.raw_user_meta_data->'ac_units', u.raw_user_meta_data->'acUnits')) = 'array'
            then coalesce(u.raw_user_meta_data->'ac_units', u.raw_user_meta_data->'acUnits')
        else p.ac_units
    end
from auth.users u
where u.id = p.id
  and (
        nullif(trim(coalesce(p.referral, '')), '') is null
        or coalesce(jsonb_array_length(p.ac_units), 0) = 0
      );
...
set ac_units = (
    select coalesce(jsonb_agg(
        jsonb_strip_nulls(jsonb_build_object(
            'key', format('legacy-unit-%s', idx),
            'image_path', nullif(p.unit_image_paths ->> (idx - 1), ''),
            'image_url', nullif(p.unit_image_urls ->> (idx - 1), ''),
    ...
    from generate_series(
        1,
        greatest(
            coalesce(jsonb_array_length(p.unit_image_paths), 0),
            coalesce(jsonb_array_length(p.unit_image_urls), 0)
        )
    ) as idx
)
where coalesce(jsonb_array_length(p.ac_units), 0) = 0
  and greatest(
        coalesce(jsonb_array_length(p.unit_image_paths), 0),
        coalesce(jsonb_array_length(p.unit_image_urls), 0)
      ) > 0;

comment on column public.profiles.referral is
'Sumber referal atau rujukan konsumen.';

comment on column public.profiles.ac_units is
'Daftar unit AC konsumen dalam format JSON terstruktur, termasuk metadata spesifikasi dan referensi foto Storage.';

comment on column public.orders.ac_type is
'Jenis AC yang dipesan, disimpan sebagai snapshot pada saat order dibuat.';

comment on column public.orders.ac_unit_key is
'Referensi key unit AC dari profiles.ac_units bila order dibuat dari unit tersimpan.';
